...
{
    public partial class uc_smallpercentagecircle : System.Web.UI.UserControl
    {
        private string name;
        ...
        public void GetResponse(string id, string cntrlName)
        {
            ...
            if (cntrlName == "CriteriaWisePercents")
            {
            ...
            if (responseObj.IsSuccessStatusCode)
            {
                ...
                if (m_Response != null)
                {
                    ...
                    //List<ProfilesVie DataTable dt
                    DataTable dt = new DataTable();
                    dt.Columns.Add("CriteriaPercentages");
                    int[] percents = { 50, 60, 70 };
                    for (int i = 0; i < 3; i++)
                    {
                        DataRow dr = dt.NewRow();
                        dr["CriteriaPercentages"] = percents[i];
                        dt.Rows.Add(dr);
                    }
                    lstsmallpercents.DataSource = dt;
                    lstsmallpercents.DataBind();
                    //lstpercentagesbottom.DataSource = wss;
                    //lstpercentagesbottom.DataBind();
                }
